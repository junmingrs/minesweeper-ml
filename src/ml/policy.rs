use burn::{Tensor, module::Module, nn::{Linear, LinearConfig, conv::{Conv2d, Conv2dConfig}}, prelude::Backend, tensor::activation};
...
impl<B: Backend> Policy<B> {
    pub fn new(device: &B::Device, height: usize, width: usize, action_size: usize) -> Self {
        Self {
            ...
            conv3: Conv2dConfig::new([64, 64], [3, 3]).with_padding(burn::nn::PaddingConfig2d::Same).init(device),
            fc1: LinearConfig::new(64 * height * width, 256).init(device),
            fc2: LinearConfig::new(256, action_size).init(device),
        }
    }
